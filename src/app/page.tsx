...
const features = [
  { icon: BarChart3, title: "Dashboard", description: "Ve tu salud financiera de un vistazo: ingresos, gastos y ahorro en tiempo real.", color: "bg-emerald-100 text-emerald-700" },
  { icon: CreditCard, title: "Transacciones", description: "Registra y categoriza cada ingreso y gasto en segundos.", color: "bg-blue-100 text-blue-700" },
  { icon: Wallet, title: "Presupuesto", description: "Pon límites por categoría y sabe exactamente en qué gastas.", color: "bg-violet-100 text-violet-700" },
  { icon: PiggyBank, title: "Metas de ahorro", description: "Crea objetivos, haz seguimiento y celebra cada avance.", color: "bg-amber-100 text-amber-700" },
  { icon: TrendingUp, title: "Deudas", description: "Controla lo que debes y simula cómo liquidarlo antes.", color: "bg-red-100 text-red-700" },
  { icon: Calculator, title: "Calculadoras", description: "Interés compuesto, hipoteca, FIRE, inflación y más.", color: "bg-teal-100 text-teal-700" },
];
...
const jsonLd = {
  ...
  author: { "@type": "Person", name: "Jose Antonio Martos" },
};
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 justify-center mb-9 text-sm text-gray-500">
              <span className="flex items-center justify-center gap-1.5"><CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" />Configúralo en 2 minutos</span>
              <span className="flex items-center justify-center gap-1.5"><CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" />Tus datos, siempre seguros</span>
            </div>
      ...
      <section className="py-10 sm:py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-4xl font-bold text-gray-900 mb-3">Todo lo que necesitas para mejorar tu situación financiera</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
            {features.map((feature) => {
              const Icon = feature.icon;
              return (
                <div key={feature.title} className="flex items-start gap-4 p-4 sm:p-6 rounded-2xl bg-gray-50 sm:bg-white sm:border sm:border-gray-100 hover:border-emerald-200 hover:shadow-md transition-all sm:flex-col sm:items-start">
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 shadow-sm ${feature.color}`}><Icon className="w-5 h-5" /></div>
                  <div>
                    <h3 className="text-sm sm:text-base font-bold text-gray-900 mb-1 leading-tight">{feature.title}</h3>
      ...
      <section className="py-12 sm:py-20 bg-gradient-to-r from-emerald-600 to-teal-600">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h2 className="text-2xl sm:text-4xl font-bold text-white mb-3 leading-tight">Haz más con lo que ya tienes.</h2>
          <p className="text-sm sm:text-lg text-emerald-100 mb-2">Deja de improvisar. Controla, planifica y ahorra.</p>
          <p className="text-xs sm:text-sm text-emerald-200 mb-7">Solo tarda 2 minutos.</p>
          <Link href="/register" className="inline-flex items-center justify-center gap-2 bg-white text-emerald-700 font-bold px-8 py-3.5 rounded-xl hover:bg-emerald-50 transition-all text-base shadow-lg">
